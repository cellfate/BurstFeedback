...
save_folder  = fullfile(pwd,'results/validationFeedback');
...
%% collect result
clear;clc;
genefiles = dir('results\validation_Feedback\*.mat');
results = zeros(length(genefiles),13);
for i = 1:length(genefiles)
    s = genefiles(i).name;
    s = str2num(s(isstrprop(s,'digit')));
    filename = sprintf('results\\validation_Feedback\\%s',genefiles(i).name);
    load(filename);
    results(s,:) = result_validation;
    disp(i)
end

csvwrite('results\validation_Feedback_results.csv',results)

results = csvread('results\validation_Feedback_results.csv');

results_po_2 = results(results(:,4) < 0 & results(:,3) == 2,:);
results_po_10 = results(results(:,4) < 0 & results(:,3) == 10,:);
results_po_100 = results(results(:,4) < 0 & results(:,3) == 100,:);
results_po_900 = results(results(:,4) < 0 & results(:,3) == 900,:);

results_ne_2 = results(results(:,4) > 0 & results(:,3) == 2,:);
results_ne_10 = results(results(:,4) > 0 & results(:,3) == 10,:);
results_ne_100 = results(results(:,4) > 0 & results(:,3) == 100,:);
results_ne_900 = results(results(:,4) > 0 & results(:,3) == 900,:);

f = figure;
subplot(2,3,1)
validationPlot(results_po_2,[-12,3]);

subplot(2,3,2)
validationPlot(results_po_10,[-12,3]);

subplot(2,3,3)
validationPlot(results_po_100,[-12,3]);

subplot(2,3,4)
validationPlot(results_ne_2,[-12,3]);

subplot(2,3,5)
validationPlot(results_ne_10,[-12,3]);

subplot(2,3,6)
validationPlot(results_ne_100,[-12,3]);
