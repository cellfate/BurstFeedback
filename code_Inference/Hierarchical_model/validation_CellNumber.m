...
theta_true_set = [3 4 0 0;
                  ...
                  3 18 2 3;
                  5 10 10 3];
num_samples_set = [200,300,500,1000,5000];

param_set = [];
for num = num_samples_set
    for theta_index = 1:size(theta_true_set,1)
        mat_temp = [num,theta_true_set(theta_index,:)];
        param_set = [param_set;[[1:50]',repmat(mat_temp,50,1)]];
    end
end

save_folder  = fullfile(pwd,'results/validation_CellNumber/');
...
parfor iter = 1:size(param_set,1)
    ...
    [data,~] = generateSample(theta_true,m_max,num_samples);
    data_mean = mean(data);
    data_var = var(data);
    ...
    % collect
    result_validation = [times,num_samples,theta_true,theta_est];
    
    filename = sprintf('validation_cellnumber=%d_a=%d_b=%d_k=%d_h=%d_%d.mat',num_samples,a,b,k,h,times);
    parsaveValidation([save_folder,filename],results_total,result_validation);
    fprintf('validation of %d iters complete!\n',iter);
end

clear;clc;
genefiles = dir('results\validation_CellNumber\*.mat');
results = [];
for i = 1:length(genefiles)
    s = genefiles(i).name;
    filename = sprintf('results\\validation_CellNumber\\%s',s);
    load(filename);
    results = [results;result_validation];
    disp(i)
end

csvwrite('results\validation_CellNumber_results.csv',results)
